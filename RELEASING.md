...
- Obtain an Apple Developer ID Application certificate and notarization
  credentials. Ad-hoc signatures are for local testing only.
...
5. Sign the macOS executable with Developer ID, enable hardened runtime, add a
   secure timestamp, notarize the archive, and verify it on a clean Mac.
